"""
Qwen2.5-3B-Instruct AI Agent — Railway uchun (papkasiz/flat versiya)
======================================================================
Bu FastAPI ilova quyidagilarni bajaradi:
  ...
  3. Shu papkadagi `skill__*.md` fayllarini (jami 300+) indekslaydi va foydalanuvchi
     xabariga eng mos keladigan 1-2 tasini topib, kontekstga qo'shadi
  4. OpenAI-mos /v1/chat/completions endpoint'ini tashqariga chiqaradi

Eslatma: bu versiyada HECH QANDAY subpapka ishlatilmaydi — barcha skill
fayllari shu ilova bilan bir qatorda, `skill__<kategoriya>__<nomi>.md`
nomlanishida saqlanadi (masalan: skill__academic__academic-historian.md).
"""
...
# ----------------------------------------------------------------------------
# Skill indeksi — shu papkadagi barcha skill__*.md fayllarini o'qib,
# nomi + tavsifi bo'yicha kalit so'z indeksini quradi
# ----------------------------------------------------------------------------
class SkillIndex:
    def __init__(self, root: Path):
        self.root = root
        self.entries = []  # [{path, name, description, tags, search_blob}]
        self._build()
    ...
    def _build(self):
        for path in sorted(glob.glob(str(self.root / SKILL_FILE_PATTERN))):
            p = Path(path)
            try:
            ...
            if not name:
                # skill__category__skill-name.md -> "category skill-name"
                stem = p.stem.replace("skill__", "").replace("__", " ")
                name = stem
            blob = f"{p.stem} {name} {description} {tags}".lower()
            self.entries.append(
                {
                    "path": p,
                    "name": name,
        ...
        print(f"[skills] {len(self.entries)} ta skill fayli indekslandi ({self.root})")
...
def on_startup():
    ...
    skill_index = SkillIndex(BASE_DIR)
